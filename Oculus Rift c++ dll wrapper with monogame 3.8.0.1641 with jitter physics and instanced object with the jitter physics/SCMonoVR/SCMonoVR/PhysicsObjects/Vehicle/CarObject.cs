...
namespace JitterDemo.Vehicle
{
    public class CarObject : DrawableGameComponent
    {
        ...
        #region Draw Wheels
        private void DrawWheels(Matrix view, Matrix projection)
        {
            JitterDemo demo = this.Game as JitterDemo;
            ...
            for(int i = 0;i<carBody.Wheels.Length;i++)
            {
                ...
                foreach (ModelMesh mesh in tireModel.Meshes)
                {
                    foreach (BasicEffect effect in mesh.Effects)
                    {
                        ...
                        effect.Projection = projection;// demo.Camera.Projection;
                    }
                    mesh.Draw();
                }
        ...
        private void DrawChassis(Matrix view, Matrix projection)
        {
            JitterDemo demo = this.Game as JitterDemo;
        ...
        {
            chassisModel = this.Game.Content.Load<Model>("Model/car");
            tireModel = this.Game.Content.Load<Model>("Model/wheel");

            base.LoadContent();
        }

        public void Draw(GameTime gameTime, Matrix view, Matrix projection) //override
        {
            DrawWheels(view, projection);
            DrawChassis(view, projection);
            base.Draw(gameTime);
        }
